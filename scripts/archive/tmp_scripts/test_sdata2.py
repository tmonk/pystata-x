...
# Test _bist_sdata with 3 pushes
sp_before = eng._save_sp()
eng._push_int(1)          # first push: obs
eng._push_int(1)          # second push: var
eng._push_str(b"")        # third push: result (dummy)
...
if sp_after > sp_before:
    ...
    print(f"  no change: {sp_after} <= {sp_before}")
