...
use rather than eagerly at ``import pystata_x`` time. This keeps a bare
``from pystata_x.stata_setup import config`` fast on the experimental branch.
"""
...
def __getattr__(name: str):
    """Lazy-import heavy public attributes and the package version."""
    if name == "__version__":
        try:
    ...
    lazy_map: dict[str, tuple[str, str | None]] = {
...
def _import_module(mod_name: str) -> None:
    ...
    _LazyModules[mod_name] = importlib.import_module(mod_name)
